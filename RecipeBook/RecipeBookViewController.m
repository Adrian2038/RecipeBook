...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"showRecipeDetail"]) {
        if ([segue.destinationViewController isKindOfClass:[RecipeDetailViewController class]]) {
            ...
            desController.recipe = [recipes objectAtIndex:indexPath.row];
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    RecipeBookTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
    
    if (!cell) {
        cell = [[RecipeBookTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
    ...
    cell.recipePrepLabal.text = recipe.prepTime;
...
- (void)filterContentWithSerachText:(NSString *)searchText scope:(NSString *)scope
{
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"name contains[c] %@", searchText];
    searchResults = [recipes filteredArrayUsingPredicate:predicate];
}

#pragma makk - Search Delegate

-(BOOL)searchDisplayController:(UISearchDisplayController *)controller
shouldReloadTableForSearchString:(NSString *)searchString
{
    [self filterContentWithSerachText:searchString
